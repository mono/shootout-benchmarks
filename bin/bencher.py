...
# $Id: bencher.py,v 1.9 2012/06/14 20:07:17 igouy-guest Exp $
...
if sys.platform == 'win32':
   ...
   def linkToSource(directory,filename,dstDir=None,srcFilename=None):
      ...
      if srcFilename == None:
         ...
         src = join(directory,srcFilename)

      # tmpdir is $CWD
      if dstDir == None:
         dst = join(filename)
      else:
         dst = join(dstDir,filename)
...
def appendToBothDataCsv(ms,fp,df,ndf):
   ...
   for each in ms:
      ...
      if not v or (each.isOkay() and each.time['elapsed'] < v.time['elapsed']):
         d[each.arg] = each
      ...
      if not each.isOkay():
         each.time = dict(user=0, system=0)
         each.ctxswitches = dict(voluntary=0, involuntary=0)
         each.maxMem = 0
...
# def writeHeader(f):
#    f.write('name,lang,id,n,size(B),user(s),system(s),total(s),mem(KB),status,load,elapsed(s)\n')

def writeHeader(f):
   f.write('name,lang,id,record\n')


def makeSummaryFiles(iniName):
   os.chdir(dirs['tmp'])

   datacsv = 'filtered_measurements.dat'
   ndatacsv = 'fastest_measurements.dat'
   bulkdatacsv = 'all_measurements.dat'
