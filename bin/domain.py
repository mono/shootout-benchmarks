...
class Record(object):
   ...
   def __init__(self,arg='0'):
      self.arg = 0
      self.time = dict(user=0.0, system=0.0, elapsed=0.0)
   ...
   def fromString(self,s):
      ...
      self.arg = a['arg']
      self.time = a['time']
      self.ctxSwitches = a['ctxSwitches']
      self.maxMem = a['maxMem']
      self.status = a['status']
      self.cpuLoad = a['cpuLoad']
      return self
   ...
   def __cmp__(self,other):
      return \
        ...
        -1 if self.status > other.status else (
         1 if self.status < other.status else (
        -1 if self.time['user'] + self.time['system'] < other.time['user'] + other.time['system'] else (
         1 if self.time['user'] + self.time['system'] > other.time['user'] + other.time['system'] else (
         0 )) )) ))
   ...
   def hasExceeded(self,cutoff):
      return self.time['user'] + self.time['system'] > cutoff
